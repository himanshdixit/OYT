...
import b11 from '/public/images/pagebanners/winepc.jpg';
import b12 from '/public/images/pagebanners/winemb.jpg';
...
const Winedetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      // const url="http://localhost:8000/api/wine_booking";
      const url="https://oyt.travel/api/wine_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!"); } )
        .catch( err => console.log(err) );
        nav("/checkout4", { state:inputValue});
    }  
        
      const [posts, setPosts] = useState([])
  
      useEffect(()=> {
          axios.get(`/api/winedetails/`+params.id)
          .then(res => {
              console.log(res)
          ...
          useEffect(()=> {
            axios.get(`/api/winedata/`+params.id)
            .then(res => {
                console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/winelist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/wine/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <b style={{color:'#ff3d00'}}>Venue : { item.type }</b> <br/>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/winedetailpage/' + item.id}><img src={'/uploads/wine/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/winedetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Wine</h4>
...
<select type="select" name="quantity" value={inputValue.quantity} onChange={handleInput} class="form-control" id="quantity" style={{fontSize:'14px'}}>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Pack Size (if any)</label>
<select type="select" name="pack" value={inputValue.pack} onChange={handleInput} class="form-control" id="pack" style={{fontSize:'14px'}}>
<option></option>
