...
import b11 from '/public/images/pagebanners/resortpc.jpg';
import b12 from '/public/images/pagebanners/resortmb.jpg';
...
const Resortdetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      //const url="http://localhost:8000/api/resort_booking";
      const url="https://oyt.travel/api/resort_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!"); } )
        .catch( err => console.log(err) );
        nav("/checkout8", { state:inputValue});
    }  
        
    const [posts, setPosts] = useState([])

    useEffect(()=> {
        axios.get(`/api/resortdetails/`+params.id)
        .then(res => {
            console.log(res)
        ...
        useEffect(()=> {
          axios.get(`/api/resortdata/`+params.id)
          .then(res => {
              console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/resortlist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/resort/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <b style={{color:'#ff3d00'}}>Location : { item.location }</b> <br/>
        <b style={{color:'#ff3d00'}}>Duration : { item.duration }</b> <br/>
        <b style={{color:'#ff3d00'}}>Counrty : { item.country }</b>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/resortdetailpage/' + item.id}><img src={'/uploads/resort/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/resortdetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Resorts</h4>
...
<input type="date" name="check_in" value={inputValue.check_in} onChange={handleInput} class="form-control" id="check_in" style={{fontSize:'14px'}} />
</div>
</div>
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Check-out</label>
<input type="date" name="check_out" value={inputValue.check_out} onChange={handleInput} class="form-control" id="check_out" style={{fontSize:'14px'}} />
</div>
</div>
</div>
...
<select type="select" name="rooms" value={inputValue.rooms} onChange={handleInput} class="form-control" id="rooms" style={{fontSize:'14px'}}>
<option value="1">01</option>
<option value="2">02</option>
<option value="3">03</option>
<option value="4">04</option>
...
<select type="select" name="type" value={inputValue.type} onChange={handleInput} class="form-control" id="type" style={{fontSize:'14px'}}>
<option></option>
...
<div class="col-4">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Food</label>
<select type="select" name="food" value={inputValue.food} onChange={handleInput} class="form-control" id="food" style={{fontSize:'14px'}}>
<option></option>
