...
import b11 from '/public/images/pagebanners/cruisepc.jpg';
import b12 from '/public/images/pagebanners/cruisemb.jpg';
...
//       const url="http://localhost:8000/api/cruise_booking";
      const url="https://oyt.travel/api/cruise_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!");} )
        .catch( err => console.log(err) );
        nav("/checkout2", { state:inputValue});
        //window.location.href = "http://localhost:8000/checkout";
        // await axios.post("http://localhost:8000/checkout",inputValue);
      }  
        
    const [posts, setPosts] = useState([])

    useEffect(()=> {
        axios.get(`/api/cruisedetails/`+params.id)
        .then(res => {
            console.log(res)
        ...
        useEffect(()=> {
          axios.get(`/api/cruisedata/`+params.id)
          .then(res => {
              console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/cruiselist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/cruise_items/'+item.image} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="profile2-tab" data-toggle="tab" href="#profile2" role="tab" aria-controls="profile2" aria-selected="false" class="nav-link  mr-sm-3 " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Sailing Info</a>
        </li>
        <li class="nav-item flex-sm-fill">
          <a id="contact2-tab" data-toggle="tab" href="#contact2" role="tab" aria-controls="contact2" aria-selected="false" class="nav-link  " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Features</a>
        </li>
        <li class="nav-item flex-sm-fill">
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        <p align="justify" style={{ fontFamily: "'Comfortaa', sans-serif !important" }} dangerouslySetInnerHTML={{__html: item.des}}></p>
        <b style={{color:'#ff3d00'}}>Sailing Date : {item.sailing_dates}</b>
		</div>
        <div id="profile2" role="tabpanel" aria-labelledby="profile-tab" class="tab-pane fade px-2 py-2" >
        <p align="justify" dangerouslySetInnerHTML={{__html: item.sailing_info}} ></p>
        </div>
        <div id="contact2" role="tabpanel" aria-labelledby="contact-tab" class="tab-pane fade px-2 py-2">
        <p align="justify" dangerouslySetInnerHTML={{__html: item.features}} ></p>
        </div>
        <div id="contact3" role="tabpanel" aria-labelledby="contact-tab" class="tab-pane fade px-2 py-2">
        <p align="justify" dangerouslySetInnerHTML={{__html: item.policies}} ></p>
        </div>
...
<div class="uk-slider-container" uk-slider="autoplay:true;autoplay-interval: 6000;pause-on-hover: true;">
    <div class="uk-position-relative uk-visible-toggle uk-light" tabindex="-1">
        <ul class="uk-slider-items uk-child-width-1-1 uk-child-width-1-1@s uk-child-width-1-1@m uk-child-width-1-1@l uk-grid">
                ...
                <li>
                 <div class="uk-card uk-card-default">
                    <div class="uk-card-media-top">
                        ...
                        <a href={'/cruisedetailpage/' + item.id}><img src={'/uploads/cruise_items/'+item.image} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/cruisedetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Cruise</h4>
