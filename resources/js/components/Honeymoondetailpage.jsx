...
import b11 from '/public/images/pagebanners/honeymoonpc.jpg';
import b12 from '/public/images/pagebanners/honeymoonmb.jpg';
...
const Honeymoondetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      // const url="http://localhost:8000/api/honeymoon_booking";
      const url="https://oyt.travel/api/honeymoon_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!"); } )
        .catch( err => console.log(err) );
        nav("/checkout6", { state:inputValue});
    }  
        
      const [posts, setPosts] = useState([])
  
      useEffect(()=> {
          axios.get(`/api/honeymoondetails/`+params.id)
          .then(res => {
              console.log(res)
          ...
          useEffect(()=> {
            axios.get(`/api/honeymoondata/`+params.id)
            .then(res => {
                console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/honeymoonlist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/honeymoon/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="contact2-tab" data-toggle="tab" href="#contact2" role="tab" aria-controls="contact2" aria-selected="false" class="nav-link  " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Amenities</a>
        </li>
        <li class="nav-item flex-sm-fill">
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <b style={{color:'#ff3d00'}}>Day/Nights : { item.nights }</b>
        <b style={{color:'#ff3d00'}}>Travelers : { item.travelers }</b>
        </div>
        <div id="contact2" role="tabpanel" aria-labelledby="contact-tab" class="tab-pane fade px-2 py-2">
        <p align="justify" dangerouslySetInnerHTML={{__html: item.chart}}></p>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/honeymoondetailpage/' + item.id}><img src={'/uploads/honeymoon/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/honeymoondetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
