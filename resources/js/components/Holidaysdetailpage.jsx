...
import b11 from '/public/images/pagebanners/holidaypc.jpg';
import b12 from '/public/images/pagebanners/holidaymb.jpg';


const Holidaysdetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      // const url="http://localhost:8000/api/holiday_booking";
      const url="https://oyt.travel/api/holiday_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!");  } )
        .catch( err => console.log(err) );
        nav("/checkout7", { state:inputValue});
    }  
        
      const [posts, setPosts] = useState([])
  
      useEffect(()=> {
          axios.get(`/api/holidaydetails/`+params.id)
          .then(res => {
              console.log(res)
          ...
          useEffect(()=> {
            axios.get(`/api/holidaydata/`+params.id)
            .then(res => {
                console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/holidaylist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/holiday/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="profile2-tab" data-toggle="tab" href="#profile2" role="tab" aria-controls="profile2" aria-selected="false" class="nav-link  mr-sm-3 " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Daily Activity Chart</a>
        </li>
        <li class="nav-item flex-sm-fill">
...
posts.map((item, index)=>
      ...
      <div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <b style={{color:'#ff3d00'}}>Day-Night : { item.day_night }</b> <br/>
        ...
        <p align="justify" dangerouslySetInnerHTML={{__html: item.daily_chart}}></p>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/holidaysdetailpage/' + item.id}><img src={'/uploads/holiday/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/holidaysdetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Holidays</h4>
...
<div class="col-12">
<div class="form-group">
<label style={{fontSize:'14px'}}>Guide Assistant Service</label>
<select type="select" name="services" value={inputValue.services} onChange={handleInput} class="form-control" id="services" style={{fontSize:'14px'}}>
<option></option>
...
export default Holidaysdetailpage;