...
import b11 from '/public/images/pagebanners/golfpc.jpg';
import b12 from '/public/images/pagebanners/golfmb.jpg';
...
const Golfdetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      // const url="http://localhost:8000/api/golf_booking";
      const url="https://oyt.travel/api/golf_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!"); } )
        .catch( err => console.log(err) );
        nav("/checkout10", { state:inputValue});
    }  
        
      const [posts, setPosts] = useState([])
  
      useEffect(()=> {
          axios.get(`/api/golfdetails/`+params.id)
          .then(res => {
              console.log(res)
          ...
          useEffect(()=> {
            axios.get(`/api/golfdata/`+params.id)
            .then(res => {
                console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/golflist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/golf/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="profile2-tab" data-toggle="tab" href="#profile2" role="tab" aria-controls="profile2" aria-selected="false" class="nav-link  mr-sm-3 " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Ground Opening Timings</a>
        </li>
        <li class="nav-item flex-sm-fill">
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <p align="justify" dangerouslySetInnerHTML={{__html: item.timing_chart}}></p>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/golfdetailpage/' + item.id}><img src={'/uploads/golf/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/golfdetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>No. Of Players</label>
<select type="select" name="player" value={inputValue.player} onChange={handleInput} class="form-control" id="player" style={{fontSize:'14px'}}>
<option></option>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Golf Match Timings</label>
<select type="select" name="timing" value={inputValue.timing} onChange={handleInput} class="form-control" id="timing" style={{fontSize:'14px'}}>
<option></option>
<option value="07am-9am">07am-9am</option>
<option value="09am-12pm">09am-12pm</option>
<option value="01pm-05pm">01pm-05pm</option>
<option value="07pm-10pm">07pm-10pm</option>
</select>
</div>
</div>
</div>
