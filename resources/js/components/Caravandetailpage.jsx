...
import b11 from '/public/images/pagebanners/caravanpc.jpg';
import b12 from '/public/images/pagebanners/caravanmb.jpg';
...
const Caravandetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
     ...
     //const url="http://localhost:8000/api/caravan_booking";
     const url="https://oyt.travel/api/caravan_booking";
    const res = await axios.post(url,inputValue)
      .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!");} )
      .catch( err => console.log(err) );
      nav("/checkout5", { state:inputValue});
    }      
        
    const [posts, setPosts] = useState([])

    useEffect(()=> {
        axios.get(`/api/caravandetails/`+params.id)
        .then(res => {
            console.log(res)
        ...
        useEffect(()=> {
          axios.get(`/api/caravandata/`+params.id)
          .then(res => {
              console.log(res)
...
{/* <img src={banner2} class="img-fluid" style={{width:'100%'}} alt="oyt" /> */}
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/caravanlist/'}> Products </Link>  /</b> 

{ 
posts.map((item)=>
<b style={{color:'rgb(255, 61, 0)'}}>{item.name}</b> 
)}  
</div>
</div>
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/caravan/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="profile2-tab" data-toggle="tab" href="#profile2" role="tab" aria-controls="profile2" aria-selected="false" class="nav-link  mr-sm-3 " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Highlights</a>
        </li>
        <li class="nav-item flex-sm-fill">
          <a id="contact2-tab" data-toggle="tab" href="#contact2" role="tab" aria-controls="contact2" aria-selected="false" class="nav-link  " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Amenties</a>
        </li>
        <li class="nav-item flex-sm-fill">
          <a id="contact3-tab" data-toggle="tab" href="#contact3" role="tab" aria-controls="contact3" aria-selected="false" class="nav-link  " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Destinations</a>
        </li>
      </ul>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/caravandetailpage/' + item.id}><img src={'/uploads/caravan/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/caravandetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Caravans</h4>
...
<form onSubmit={handleSubmit} enctype="multipart/form-data" class="info-box">

<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Name</label>
<input type="name" name="name" value={inputValue.name} onChange={handleInput} class="form-control" id="name" style={{fontSize:'14px'}} />
</div>
</div>
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Phone</label>
<input type="number" name="phone" value={inputValue.phone} onChange={handleInput} class="form-control" id="phone" style={{fontSize:'14px'}} />
</div>
</div>
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Email</label>
<input type="email" name="email" value={inputValue.email} onChange={handleInput} class="form-control" id="email" style={{fontSize:'14px'}} placeholder="e.g. abc@gmail.com" />
</div>
</div>
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Gender</label>
<select type="select" name="gender" value={inputValue.gender} onChange={handleInput} class="form-control" id="gender" style={{fontSize:'14px'}}>
<option></option>
...
<select type="select" name="adult" value={inputValue.adult} onChange={handleInput} class="form-control" id="adult" style={{fontSize:'14px'}}>
<option></option>
...
<select type="select" name="child" value={inputValue.child} onChange={handleInput} class="form-control" id="child" style={{fontSize:'14px'}}>
<option></option>
<option value="1">01</option>
<option value="2">02</option>
<option value="3">03</option>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Pick-up Location</label>
<select type="select" name="pickup_location" value={inputValue.pickup_location} onChange={handleInput} class="form-control" id="pickup_location" style={{fontSize:'14px'}}>
<option></option>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Drop-up Location</label>
<select type="select" name="drop_location" value={inputValue.drop_location} onChange={handleInput} class="form-control" id="drop_location" style={{fontSize:'14px'}}>
<option></option>
<option value="Delhi">Delhi</option>
<option value="Uttar Pradesh">Uttar Pradesh</option>
<option value="Maharastra">Maharastra</option>
...
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Driver Choice</label>
<select type="select" name="driver" value={inputValue.driver} onChange={handleInput} class="form-control" id="driver" style={{fontSize:'14px'}}>
<option></option>
...
<div style={{textAlign:'center'}}>
<button class="btn btn-primary" name="submit" value="submit" type="submit" style={{fontSize:'14px',marginLeft:'5px',lineHeight:'1',border:'2px solid #ff3d00',backgroundColor:'#ff3d00',borderRadius:'5px'}}>Submit</button>
</div>
</form>
</div>
</div>
