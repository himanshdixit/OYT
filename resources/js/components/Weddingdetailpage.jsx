...
import b11 from '/public/images/pagebanners/weddingpc.jpg';
import b12 from '/public/images/pagebanners/weddingmb.jpg';
...
const Weddingdetailpage=()=>{
  ...
  function getDate() {
    ...
    const handleSubmit= async (e)=>{
      ...
      // const url="http://localhost:8000/api/wedding_booking";
      const url="https://oyt.travel/api/wedding_booking";
      const res = await axios.post(url,inputValue)
        .then( resp => { console.log(resp); alert("Data Submitted successfully... Thank you !!");} )
        .catch( err => console.log(err) );
        nav("/checkout3", { state:inputValue});
    }  
        
    const [posts, setPosts] = useState([])

    useEffect(()=> {
        axios.get(`/api/weddingdetails/`+params.id)
        .then(res => {
            console.log(res)
        ...
        useEffect(()=> {
          axios.get(`/api/weddingdata/`+params.id)
          .then(res => {
              console.log(res)
...
<b style={{ padding:'10px' }}> <Link to={'/'}> Home </Link> / <Link to={'/weddinglist/'}> Products </Link>  /</b> 
...
posts.map((item)=>
    <div class="carousel-item active">
      <img src={'/uploads/weddings/'+item.photo} class="d-block w-100" alt="oyt"/>
    </div>
    )
}
...
<ul id="myTab2" role="tablist" class="nav nav-tabs nav-pills with-arrow lined flex-sm-row text-center">
        ...
        <li class="nav-item flex-sm-fill">
          <a id="profile2-tab" data-toggle="tab" href="#profile2" role="tab" aria-controls="profile2" aria-selected="false" class="nav-link  mr-sm-3 " style={{border:'1px solid lightgrey',padding: '0.1rem 0.7rem',margin:'3px'}}>Venue & Banquets</a>
        </li>
        <li class="nav-item flex-sm-fill">
...
<div id="home2" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade px-2 py-2 show active">
        ...
        <b style={{color:'#ff3d00'}}>Venue : { item.venue }</b> <br/>
        <b style={{color:'#ff3d00'}}>Banquets : { item.banquets }</b>
        </div>
    ...
    data.map((item)=>
                <li>
                 <div class="uk-card uk-card-default">
                    ...
                    <a href={'/weddingdetailpage/' + item.id}><img src={'/uploads/weddings/'+item.photo} width="1800" height="1200" alt="oyt" /></a>
                    ...
                    <a href={'/weddingdetailpage/' + item.id}><h5 class="uk-card-title"> {capitalizeWords(item.name)} </h5></a>
...
<h4 style={{marginTop:'10px',fontWeight:'700',color:'#3c77c9',lineHeight:'1',marginBottom:'10px'}}>Weddings</h4>
...
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Wedding-Date</label>
<input type="date" name="wedding_date" value={inputValue.wedding_date} onChange={handleInput} class="form-control" id="wedding_date" style={{fontSize:'14px'}} />
</div>
</div>
...
<div class="col-12">
 <div class="form-group">
<label style={{fontSize:'14px'}}>No. of People</label>
<select type="select" name="people" value={inputValue.people} onChange={handleInput} class="form-control" id="people" style={{fontSize:'14px'}}>
...
<div class="col-6">
 <div class="form-group">
<label style={{fontSize:'14px'}}>Food Pax Type</label>
<select type="select" name="food_pax_type" value={inputValue.food_pax_type} onChange={handleInput} class="form-control" id="food_pax_type" style={{fontSize:'14px'}}>
